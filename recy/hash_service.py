...
from src.simple_object_backup_to_pan_baidu.utils import DbMixin, retry_decorator, ServiceStatusTable, logger_configurer
from src.simple_object_backup_to_pan_baidu.config import get_config, get_logger_queue
...
class FormatTransformer:
    @staticmethod
    def scan_record_to_manual_review_object(scan_record: ScanRecords, reason: str):
    ...
    @staticmethod
    def scan_record_to_hash_result(scan_record: ScanRecords, hash_dict: dict):
        return HashResult(
            host_name=scan_record.host_name,
            object_path=scan_record.object_path,
            object_type=scan_record.object_type,
...
class ObjectHash:
    ...
    def hash_file(self, scan_record: ScanRecords) -> HashResult:
        hash_result = {}
    ...
    def hash_directory(self, scan_record: ScanRecords) -> HashResult:
        hash_result = {}
    ...
    def hash_object(self, scan_record: ScanRecords):
...
class HashService:
    ...
    def _get_next_waiting_record(self) -> Optional[ScanRecords]:
        """Get the next waiting record from scan_results table"""
        self.logger.debug("Fetching next waiting record...")
        try:
            with Session(self.db_engine) as session:
                record = session.query(ScanRecords).filter(
                    ScanRecords.status == "waiting"
                ).order_by(ScanRecords.id).first()
                return record
        except exc.OperationalError as e:
    ...
    def _update_scan_status(
        ...
        try:
            with Session(self.db_engine) as session:
                record = session.query(ScanRecords).filter(ScanRecords.id == scan_id).first()
                if record:
    ...
    def _submit_hash_task(self, scan_record: ScanRecords) -> None:
    ...
    def _process_hash(
        ...
        from .hash_worker import process_hash_task # type: ignore
    ...
    def _cancel_all_tasks(self) -> None:
        ...
        try:
            with Session(self.db_engine) as session:
                processing_records = session.query(ScanRecords).filter(
                    ScanRecords.status == "processing"
                ).all()
